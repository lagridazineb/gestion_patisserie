      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <h3 className="font-fraunces text-lg text-diana-cream">Production du jour — {isPatisserie ? (prepTab === 'tranche' ? 'Tranche' : prepTab === 'entremets' ? 'Entremets circulaires' : 'Gâteau par kg') : atelierLabel}</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {visibleProducts.length === 0 && (
              <p className="text-sm italic text-diana-brownLight sm:col-span-2 lg:col-span-3">Aucun produit configuré pour cet atelier pour le moment.</p>
            )}
            {visibleProducts.map((p) => {
              const qty = getTodayProduction(p.id)
              const isLow = qty <= 0
              return (
                <div key={p.id} className="flex items-center justify-between gap-3 bg-diana-dark/50 border border-diana-border/30 rounded-xl px-4 py-3">
                  ...
                  <span className={`text-sm font-semibold shrink-0 ${isLow ? 'text-diana-danger' : 'text-diana-gold'}`}>{qty} {p.unit === 'kg' ? 'kg' : ''}</span>
                </div>
              )
            })}
          </div>
